...
func main() {
	...
	logger, err := newControllerLogger(options, os.Stderr)
	parser.FatalIfErrorf(err)
	...
	if len(options.WebhookCertPath) > 0 {
		...
		webhookServerOptions.CertDir = options.WebhookCertPath
		webhookServerOptions.CertName = options.WebhookCertName
		webhookServerOptions.KeyName = options.WebhookCertKey
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "8719486c.meigma.io",
