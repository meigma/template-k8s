...
- Moon is the task front door for this template. Use `moon run root:generate`, `moon run root:manifests`, `go test ./...`, `moon run root:lint`, `moon ci --summary minimal`, and `moon run root:test-e2e` for controller/API changes.
