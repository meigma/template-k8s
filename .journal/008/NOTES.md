...
## 2026-05-19 21:37 — Release 0.1.1 execution and recovery
Goal for the session: Merge the Release Please PR, observe the release process, and make sure it passes.
Current state of the world: Merged PR #14, `chore(master): release 0.1.1`, as `1d2db99`. Release Please then created tag `v0.1.1`, a draft release, and the tag-triggered release run `26140752270`. The first real release run published binaries, container image, and the Helm chart, but failed in `Helm Chart Release` during `actions/attest` with `No credentials found for registry ghcr.io`. Root cause: Helm was logged into GHCR, but `actions/attest` needs Docker-style GHCR credentials when `push-to-registry: true`.
Recovery: Implemented and merged PR #21, `fix(release): authenticate helm chart attestations`, as `8ed4b16`. The fix adds Docker GHCR login before chart attestation and lets the chart push step recover the digest with `helm show chart` if the same chart version was already published by an earlier failed run. Manually reran `Release` from `master` for `v0.1.1`; run `26141230186` passed end to end.
Verification: Draft release `v0.1.1` has 9 assets: `checksums.txt`, four binaries, and four SBOMs. `docker buildx imagetools inspect ghcr.io/meigma/template-k8s:v0.1.1` resolves image index digest `sha256:fe721300df5d0e11d07b4304f51c9222618ad65a42e9578aff842b2341961f46`. `helm show chart` and `helm pull` work for `oci://ghcr.io/meigma/template-k8s/chart --version 0.1.1`, resolving chart digest `sha256:922e658eaf230bdf828fd0da4a50db013f7ed73c68714e0b9726b9e357e46da4`; `helm template` against the remote OCI chart also succeeds. GitHub attestation verification passed for the image, Helm chart, and all 8 checksum-listed release assets using the recovery run's actual source ref `refs/heads/master`.
Follow-up: The release remains a draft for human inspection. Current workflow annotations warn that pinned `actions/upload-artifact` and `actions/download-artifact` run on Node.js 20, which GitHub will force-upgrade to Node.js 24 by default on 2026-06-02 and remove on 2026-09-16; update those pinned actions before that becomes noisy or risky.

## 2026-05-19 21:48 — Published release Helm smoke
Goal for the session: Smoke-check the published `v0.1.1` release artifacts after the draft release was published, with focus on deploying the published Helm chart and operator image together.
Current state of the world: Created a disposable Kind cluster `template-k8s-release-smoke`, installed the published chart with `helm upgrade --install template-k8s oci://ghcr.io/meigma/template-k8s/chart --version 0.1.1 --namespace template-k8s-system --create-namespace --wait`, and confirmed the pulled chart digest was `sha256:922e658eaf230bdf828fd0da4a50db013f7ed73c68714e0b9726b9e357e46da4`. The controller Deployment rolled out `1/1` using `ghcr.io/meigma/template-k8s:v0.1.1`, and CRD `nginxdeployments.example.meigma.io` was established.
Verification: Applied the sample `NginxDeployment` into `template-k8s-smoke`; the custom resource reached `Available=True` with reason `DeploymentReady`, and the owned Deployment reached `1/1` ready. The operator created the expected same-named ConfigMap, Deployment, and Service. A local port-forward to `svc/nginx-sample` returned `hello from template-k8s`, proving the reconciled workload served the configured response. Deleted the disposable Kind cluster after the smoke.

## 2026-05-19 21:57 — Close
Goal for the session: Close session 008 after landing and verifying the release work.
Current state of the world: Session implementation PRs were merged via GitHub squash merge: PR #19 `ci(release): publish helm chart to ghcr`, PR #20 `ci(release): build containers on native arm runners`, PR #14 `chore(master): release 0.1.1`, and PR #21 `fix(release): authenticate helm chart attestations`. Local `master` is clean, tracks `origin/master`, and is fast-forwarded to `8ed4b16`. No non-journal branch tracks `.journal`, and the only remaining Worktrunk worktree is the personal journal worktree. The published `v0.1.1` release has been smoke-tested from GHCR chart plus GHCR operator image.
Handoff: Wrote `.journal/008/SUMMARY.md`, added the session row to `.journal/INDEX.md`, and updated `.journal/TECH_NOTES.md` with the durable Helm OCI release/attestation note. PR #22, `chore(master): release 0.1.2`, remains open and green as the normal next Release Please PR after the attestation recovery fix.
